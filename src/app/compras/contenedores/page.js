...
const ESTADOS = {
  ...
  TRANSITO:  { label: 'En trÃ¡nsito', color: 'badge-info',     orden: 3 },
      ...
      <div className="card-body p-4">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-0 space-y-1">
            ...
            {/* Ãšltimo evento de tracking */}
            {evento && (
              <p className="text-xs text-base-content/60">
                ðŸ“ {evento.desc}
                {evento.fecha && <span className="text-base-content/40 ml-1">Â· {fmtFecha(evento.fecha)}</span>}
              </p>
            )}
            ...
            {imp.etaEstimada && (
              <p className="text-xs text-primary font-medium">
                ðŸŽ¯ ETA estimada: {fmtFecha(imp.etaEstimada)}
              </p>
            )}
            ...
            {esBorrador && !imp.trackingActivo && (
              <p className="text-xs text-warning flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" /> Borrador sin tracking â€” escaneo desde tablet
              </p>
            )}
          </div>
          ...
          <div className="flex gap-1 shrink-0 items-start flex-wrap justify-end">
            <button
              ...
              title="El contenedor ha llegado â€” detiene el tracking y los avisos WhatsApp"
            >
              <PackageCheck className="w-3 h-3" /> LlegÃ³
            </button>
            <Link
...
export default function ContenedoresPage() {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Â¿Eliminar este contenedor del listado?')) return;
    await fetch(`/api/importaciones/${id}`, { method: 'DELETE' });
    mutate('/api/importaciones');
  };

  const handleMarcarRecibido = async (id) => {
    if (!confirm('Â¿Marcar como recibido? Se desactivarÃ¡ el tracking automÃ¡tico y dejarÃ¡s de recibir avisos WhatsApp para este contenedor.')) return;
  ...
  // Contador de trackers Ship24 â€” leÃ­do desde la API (persistente aunque se borren registros)
    ...
    <div className="container mx-auto p-6 max-w-4xl">
          ...
          <div>
            ...
            <p className="text-sm text-base-content/50">
              {conTracking > 0
                ...
                : 'Seguimiento de importaciones y cÃ¡lculo de costes'}
            </p>
            {usoShip24 && (
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                ...
                {editandoUso ? (
                  <div className="flex items-center gap-1">
                    ...
                    />
                    <button onClick={handleCorregirUso} className="btn btn-xs btn-primary">âœ“</button>
                    <button onClick={() => setEditandoUso(false)} className="btn btn-xs btn-ghost">âœ•</button>
                  </div>
                ) : (
                  <button
        ...
        <div className="flex gap-2 flex-wrap">
          {conTracking > 0 && (
            <button
              ...
              title="Forzar comprobaciÃ³n de tracking ahora"
            >
              <RefreshCw className={`w-4 h-4 ${sincronizando ? 'animate-spin' : ''}`} />
              {sincronizando ? 'Sincronizandoâ€¦' : 'Sincronizar ahora'}
            </button>
          )}
          <Link href="/herramientas/calculadora-contenedor" className="btn btn-outline btn-sm gap-1">
            <Calculator className="w-4 h-4" /> Nuevo cÃ¡lculo de gastos
          </Link>
      ...
      {!isLoading && ordenados.length === 0 && (
        <div className="text-center py-20 text-base-content/30">
          ...
          <p className="text-lg font-medium">No hay contenedores todavÃ­a</p>
          <p className="text-sm mt-1 mb-6">Crea un rastreador cuando hagas un pedido de importaciÃ³n</p>
                ...
                <div className="card-body p-3">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div className="flex items-center gap-2 flex-wrap text-sm">
                      ...
                      <span className="font-medium">{imp.descripcion || imp.numContenedor || imp.numFactura || 'â€”'}</span>
