...
export default function CalculadoraPage() {
  ...
  const [ancho, setAncho] = useState(''); // Inicializar vacío
  const [largo, setLargo] = useState(''); // Inicializar vacío
  ...
  const currentCalculation = useMemo(() => {
    ...
    if (!tarifas || !selectedMaterial || !selectedEspesor || unidadesInt <= 0 || anchoMm <= 0 || largoMm <= 0) {
      return { precioUnitario: 0, pesoUnitario: 0, precioTotal: 0, pesoTotal: 0, isValid: false };
    }

    // Comparación flexible o convirtiendo ambos a Number
    ...
    const largoMetros = largoMm / 1000;
