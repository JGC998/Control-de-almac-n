...
export default function CreatePedidoForm({ initialData = null }) {
  const router = useRouter();
  const [clienteId, setClienteId] = useState(initialData?.clienteId || '');
  const [clienteBusqueda, setClienteBusqueda] = useState(initialData?.cliente?.nombre || '');
  const [items, setItems] = useState(initialData?.items || []);
  const [notes, setNotes] = useState(initialData?.notas || ''); // Cambiado de notes a notas
  const [error, setError] = useState(null);
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setNotes(initialData.notas || ''); // Cambiado de notes a notas
  ...
  const handleItemChange = (index, field, value) => {
    ...
    if (field === 'productId') { // Ahora se llama productId
      ...
      if (producto) {
        ...
        item.pesoUnitario = producto.pesoUnitario || 0; // Añadir peso unitario del producto base
      }
  ...
  const addItem = (producto) => {
    if (!producto) {
        ...
        setItems(prev => [...prev, { plantilladId: '', description: '', quantity: 1, unitPrice: 0, productId: null, pesoUnitario: 0 }]);
        return;
    }
  ...
  // Función para recalcular precios (copiada de CreatePedidoForm original)
  const handleRecalculatePrices = async () => {
    ...
    try {
      ...
      setItems(calculatedItems.map((item, index) => ({
        ...items[index], // Mantener descripción, etc.
        unitPrice: item.unitPrice,
      })));
    } catch (err) {
      setError(err.message);
  ...
  const handleSubmit = async (e) => {
    ...
    const orderData = {
      clienteId,
      items: items.map(item => ({
        // Usamos 'description', la API de Pedidos lo mapea a 'descripcion'
        description: item.description,
        productId: item.productId || null,
        quantity: parseInt(item.quantity) || 0,
        unitPrice: parseFloat((item.unitPrice || 0).toFixed(2)), 
        pesoUnitario: parseFloat((item.pesoUnitario || 0).toFixed(2)), // Añadir peso unitario
      })),
      notas: notes,
      subtotal: finalSubtotal,
      tax: finalTax,
      total: finalTotal,
      estado: initialData?.estado || 'Pendiente' // Estado por defecto para Pedidos
    };

    // Cambiar endpoints a Pedidos
    const url = initialData ? `/api/pedidos/${initialData.id}` : '/api/pedidos';
    ...
    try {
      ...
      mutate('/api/pedidos');
      if(initialData) {
        mutate(`/api/pedidos/${initialData.id}`);
      }

      router.push(`/pedidos/${savedOrder.id}`); // Redirigir a la página de detalle del pedido
