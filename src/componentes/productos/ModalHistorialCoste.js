...
function Sparkline({ puntos }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 80 }}>
      ...
      <polyline points={points} fill="none" stroke="oklch(var(--p))" strokeWidth="2" strokeLinejoin="round" />
      {puntos.map((p, i) => (
        <circle key={i} cx={PAD + i * xStep} cy={y(p.precio)} r="3" fill="oklch(var(--p))" />
      ))}
...
export default function ModalHistorialCoste({ row, onClose }) {
  const url = row ? `/api/tarifas-coste-historial?material=${encodeURIComponent(row.material)}&espesor=${row.espesor}` : null;
    ...
    <div className="modal modal-open">
      <div className="modal-box max-w-lg">
        ...
        <h3 className="font-bold text-lg mb-1">
          Historial de coste — {row.material}
        </h3>
        ...
        {!isLoading && puntosOrdenados.length === 0 && (
          <p className="text-center text-base-content/40 py-8 text-sm">
            Sin historial todavía. Se registrará al guardar el próximo contenedor.
          </p>
        )}
          ...
          <>
            ...
            {variacion != null && (
              <div className={`flex items-center gap-2 text-sm mb-3 font-semibold ${
                variacion > 0 ? 'text-error' : variacion < 0 ? 'text-success' : 'text-base-content/50'
              }`}>
                {variacion > 0 ? <TrendingUp className="w-4 h-4" /> : variacion < 0 ? <TrendingDown className="w-4 h-4" /> : <Minus className="w-4 h-4" />}
                {variacion > 0 ? '+' : ''}{fmt(variacion, 1)}% vs contenedor anterior
              </div>
            )}
            ...
            {puntosOrdenados.length >= 2 && (
              <div className="bg-base-200 rounded-xl p-3 mb-4">
                <Sparkline puntos={puntosOrdenados} />
              </div>
            )}
            ...
            <div className="overflow-x-auto max-h-52">
              <table className="table table-xs w-full">
                <thead>
                  <tr>
                    <th>Fecha</th>
                    <th className="text-right">Precio coste (€/m²)</th>
                    ...
                    return (
                      <tr key={p.id} className="hover">
                        ...
                        <td className={`text-right font-mono text-xs ${
                          delta == null ? 'opacity-30' : delta > 0 ? 'text-error' : delta < 0 ? 'text-success' : 'opacity-30'
                        }`}>
